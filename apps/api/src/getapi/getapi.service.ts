...
export class GetApiService {
  ...
  async lookupPlate(plate: string) {
    ...
    const cached = await this.prisma.vehicleLookupCache.findUnique({
      where: { plate: normalizedPlate },
    });

    if (cached && this.isCacheValid(cached.fetchedAt)) {
      const cachedResponse = cached.responseJson as Record<string, unknown>;
      const rebuilt = this.rebuildFromCacheIfNeeded(
        normalizedPlate,
        cachedResponse,
      );
      if (rebuilt) {
        await this.prisma.vehicleLookupCache.update({
          where: { plate: normalizedPlate },
          data: {
    ...
    const apiKey = this.configService.get<string>('GETAPI_KEY');
    if (!apiKey) {
      throw new BadRequestException('GETAPI_KEY no configurada');
    }
  ...
  private isCacheValid(fetchedAt: Date) {
    const ttlMs = CACHE_TTL_DAYS * 24 * 60 * 60 * 1000;
    return fetchedAt.getTime() >= Date.now() - ttlMs;
  }

  private rebuildFromCacheIfNeeded(
    plate: string,
    ...
    return this.normalizeResponse(plate, raw as Record<string, unknown>);
  }
  ...
  private normalizeResponse(
    ...
    const doors =
      typeof doorsValue === 'string' ? Number(doorsValue) : doorsValue;
